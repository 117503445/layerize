...
func UploadUpdatedConfigToRegistry(ctx context.Context, updatedConfig []byte, registryURL, repository, username, password string) error {
	...
	logger.Info().Str("configDigest", configDigest).Int("configSize", len(updatedConfig)).Msg("Starting to upload updated config")
	...
	if err != nil {
		logger.Error().Err(err).Msg("Failed to upload config")
