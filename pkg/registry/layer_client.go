...
// UploadLayerStreamWithClient uploads a blob by initiating an upload and streaming the provided reader.
// The reader is streamed directly without buffering the whole content in memory.
func UploadLayerStreamWithClient(ctx context.Context, client *Client, reader io.Reader, digest string, repository string) error {
    logger := log.Ctx(ctx)
    scope := fmt.Sprintf("repository:%s:push,pull", repository)
