...
func ValidateBuiltImage(ctx context.Context, content string) error {
	...
	if err != nil {
		logger.Error().Err(err).Str("output", string(output)).Msg("Failed to execute podman pull and cat new.txt")
	...
	if lastLine != content {
		logger.Error().Str("expected", content).Str("actual", lastLine).Msg("new.txt content mismatch")
		return fmt.Errorf("new.txt content mismatch: expected %s, actual %s", content, lastLine)
	}

	logger.Info().Str("content", content).Msg("Successfully validated new.txt content")
	...
	output, err = cmd.CombinedOutput()
	if err != nil {
		logger.Error().Err(err).Str("output", string(output)).Msg("Failed to execute podman run ls")
		return fmt.Errorf("failed to execute podman run ls: %w", err)
	}

	if strings.Contains(string(output), "old.txt") {
		logger.Error().Str("output", string(output)).Msg("old.txt should not exist but was found")
		return fmt.Errorf("old.txt should not exist but was found")
	}

	logger.Info().Msg("Successfully validated old.txt does not exist")
	return nil
}